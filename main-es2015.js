...
let RoomComponent = class RoomComponent {
    constructor(vice, uservice, roomvice) {
        this.vice = vice;
    ...
    joinRoom() {
        const messageFace = { senderId: this.User._id, roomId: this.roomData._id, message: 'joined', type: 4, date: new Date() };
        const senser = this.roomMembers.filter(data => data.userId._id === this.User._id);
        if (senser.length < 1) {
            this.roomvice.joinRoom(this.roomData._id).subscribe(data => {
                if (data.length > 0) {
                    this.roomMembers = data;
                    this.vice.join(messageFace);
                }
            });
        }
        else {
    ...
    leaveRoom() {
        const messageFace = { senderId: this.User._id, roomId: this.roomData._id, message: 'left', type: 4, date: new Date() };
        const senser = this.roomMembers.filter(data => data.userId._id === this.User._id);
        if (senser.length > 0) {
            this.roomvice.leaveRoom(this.roomData._id, this.User._id).subscribe(data => {
                this.roomMembers = data;
                this.vice.leave(messageFace);
            });
        }
        else {
    ...
    getLeaveMessage() {
        this.vice.leaveNotification().subscribe(data => {
            this.allMessages.push(data);
        });
    }
    senderName(id) {
        if (id && this.roomMembers) {
            const senser = this.roomMembers.filter(data => data.userId._id === id);
            if (senser.length > 0) {
    ...
    sender() {
        if (this.input) {
            ...
            const messageFace = { senderId: this.User._id, roomId: this.roomData._id, message: this.input, type: 2, date: new Date() };
            this.vice.sendMessage(messageFace);
        }
        this.input = '';
    }
    getMembers() {
        this.roomvice.allMembers(this.roomData._id).subscribe(members => this.roomMembers = members);
    }
    getRoomData() {
        if (localStorage.roomData) {
            return this.roomData = JSON.parse(localStorage.roomData);
        }
        this.roomvice.data$.subscribe((data) => this.roomData = data);
    }
    getUserDetails() {
        this.uservice.USER$.subscribe(data => this.User = data);
    }
    sentMessages() {
        this.vice.getMessage().subscribe(data => {
            return this.allMessages.push(data);
        });
    }
    getJoinMessages() {
        this.vice.joinNotification().subscribe(data => {
            this.allMessages.push(data.message);
            this.roomMembers.push(data.member);
            console.log(JSON.stringify(this.roomMembers));
        });
    }
    getAllMessages() {
        this.roomvice.allMessages(this.roomData._id).subscribe(data => {
            if (data.length > 0) {
...
let Room2Component = class Room2Component {
    constructor(vice, roomvice, uservice) {
        this.vice = vice;
        this.roomvice = roomvice;
        this.uservice = uservice;
        this.allMessages = [];
    ...
    getPatData() {
        ...
        this.roomvice.PC$.subscribe((data) => this.patData = data);
    }
    getPcId() {
    ...
    getAllMessages() {
        ...
        this.roomvice.allPcMessages(pcId).subscribe(data => {
            if (data.length > 0) {
                this.allMessages = data;
            }
        });
    ...
    sentMessages() {
        this.vice.getPcMessage().subscribe(data => {
            return this.allMessages.push(data);
        });
    }
    ngOnInit() {
...
let DashboardComponent = class DashboardComponent {
    constructor(http, fb, uservice, roomvice) {
        this.http = http;
        this.fb = fb;
        this.uservice = uservice;
        this.roomvice = roomvice;
        ...
        if (this.newGroup.valid) {
            ...
            this.http.post('http://192.168.43.41:2050/createRoom', formData, this.httpOption)
                ...
                if (data._id) {
                    ...
                    this.createGroupSuccessMessage = `${data.name} has been successfuly created`;
                    setTimeout((() => modal.style.display = 'none'), 3000);
                }
                else {
                    this.createGroupErrorMessage = 'Group already exists, please use another name or join the group';
                }
                // return setTimeout((() => this.createGroupErrorMessage = '' ), 3000);
            });
        }
        else {
            setTimeout((() => this.startCreateRoom = false), 3000);
            this.createGroupErrorMessage = 'All fields are required';
        }
        this.startCreateRoom = false;
        return setTimeout((() => { this.createGroupErrorMessage = ''; this.createGroupSuccessMessage = ''; }), 3000);
    }
    fileProgress(file) {
        console.log('parsed');
    ...
    getAllRooms() {
        this.roomvice.allRooms().subscribe((data) => {
            this.allRooms = data;
...
let FogetPasswordComponent = class FogetPasswordComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let LoginComponent = class LoginComponent {
    constructor(fb, uservice, route, guard) {
        this.fb = fb;
        this.uservice = uservice;
        this.route = route;
        this.guard = guard;
        ...
        if (this.login.valid) {
            ...
            this.uservice.userLogin(form)
                ...
                if (collector._id) {
                    ...
                    this.uservice.userData.next(collector);
                    const url = this.guard.directedToUrl ? this.route.parseUrl(this.guard.directedToUrl) : '/dashboard';
...
let SignupComponent = class SignupComponent {
    ...
    get Sex() { return this.signup.get('sex'); }
    submitForm() {
        this.processing = true;
        this.submitted = true;
        if (this.signup.valid) {
            console.log('processing');
            const form = { username: this.Username.value, email: this.Email.value, password: this.Password.value, sex: this.Sex.value };
...
const routingComponents = [_Rooms_room_room_component__WEBPACK_IMPORTED_MODULE_3__["RoomComponent"], _User_signup_signup_component__WEBPACK_IMPORTED_MODULE_10__["SignupComponent"], _Rooms_room2_room2_component__WEBPACK_IMPORTED_MODULE_7__["Room2Component"],
...
let GuardService = class GuardService {
    ...
    leaving(message) {
        return new rxjs__WEBPACK_IMPORTED_MODULE_2__["Observable"](obs => {
            const action = confirm(message || 'Do you want to laeve?');
            obs.next(action);
        });
    }
};
